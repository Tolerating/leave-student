<template>
    <div class="mine_content">
        <mt-header class="qj_header" title="早自习请假记录">
        ...
        <transition enter-active-class="animated slideInRight">
            <div v-if="isShow_record">
                ...
                <leaveCard v-for="item in leaveRecord" :key="item.id" title="早自习请假" :status="item.status" :reason="item.reason" :sTime="item.sTime" :eTime="item.eTime" :putTime="item.time"/>
            </div>
        </transition>
...
export default {
    ...
    methods:{
        getRecord(){
            let _self = this;
            this.$ajax.get(_self.baseUrl+"/StudentLeave/zaozixiRecord",{ 
                 headers: {
        ...
        receive_c(sTime,eTime,leaveResult){
            ...
            Indicator.open("搜索中...");
            let _self = this;
            this.$ajax.get(_self.baseUrl+"/StudentLeave/retrievalZZX",{           
                params:{
