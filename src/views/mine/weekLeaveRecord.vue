<template>
    <div class="mine_content">
        <mt-header class="qj_header" title="周末请假记录">
        ...
        <transition enter-active-class="animated slideInRight">
            <div v-if="isShow_record">
                ...
                <leaveCard v-for="item in leaveRecord" :key="item.id" title="周末请假" :status="item.status" :reason="item.reason" :sTime="item.sTime" :eTime="item.eTime" :putTime="item.time"/>
            </div>
        </transition>
...
export default {
    ...
    data(){
        return{
            ...
            isShow_record:true,
            isShow_search:false,
            options:['未审核', '班主任审核', '辅导员审核','周末自动审核']
        }
    },
    components:{
        leaveCard,
    ...
    methods:{
        getRecord(){
            let _self = this;
            this.$ajax.get(_self.baseUrl+"/StudentLeave/weekdaysRecord",{ 
                 headers: {
        ...
        receive_c(sTime,eTime,leaveResult){
            ...
            Indicator.open("搜索中...");
            let _self = this;
            this.$ajax.get(_self.baseUrl+"/StudentLeave/retrievalWeek",{           
                params:{
